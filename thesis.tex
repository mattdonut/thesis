...
Advances in molecular manipulation allows for more detailed analysis of large biological molecules, such as protein and DNA.
Identification and characterization of these large molecules can lead to greater understanding of the biological systems that use them.
While current systems use time-of-flight methods to examine the mass of these polymers, photon scattering techniques could be used to study the their radius of gyration and aspect ratio.
Here we report the development of an analytic expression for the dependence of the radius of gyration of a polymer suspended in vacuum to its angular momentum, and the agreement of that expression with numerical simulations.

Polymer dynamics can also be studied in vivo, with a great deal of the behavior captured by some simple models.
Here we study cytoplasmic streaming in \emph{Drosophila} oocytes. The oocyte displays two stages of fluid movement; a slow stage that is generally chaotic and localized as well as a fast stage that is highly correlated and moves across the entire cell.
We use a model of motor proteins walking along microtubules anchored on the cell wall to describe the motions of the cytoplasm and microtubules.
The case of a single microtubule in an external fluid flow is studied both analytically and numerically, yielding a class of solutions with characteristic spatial curvatures and rotational frequencies.
We also investigate the behavior of many microtubules using simulations which include hydrodynamic interactions.
These simulations can exhibit chaotic localized motion or globaly correlated directed motion, depending on the system parameters.
Several of these parameters correspond to changes observed in the oocyte in the transition from slow to fast streaming, suggesting possible mechanisms for regulating streaming.
...
I would like to thank my dissertation committee members, my advisor Josh Deutsch, Bill Saxton, and Peter Young, for their time and patience in my writing process.
As my advisor, Josh needs special thanks for making sure I had projects to keep me moving towards my eventual goal, and for the immense help given in the completion of those projects.

Additionally, thanks to Bill Saxton and Corey Monteith for their hard work collecting such amazing biological data for us to study, without them none of this would be possible.

Thanks to Jennifer Raab for all of her help throughout my graduate career, especially for making sure I had the resources to continue on.

And finally thanks to my friends and family, for being so supportive in everything that I do.
...
Polymers are in general long and thin, as well as flexible with some stiffness constant.
...
investigations~\cite{DeutschPolyVac,Kleinert,mossa,DeutschCerf,Taylor,DeutschExactVac}  and show
...
In this chapter, we further investigate the equilibrium statistics
...
Following previous work ~\cite{laliena} we begin with the entropy as a function of the total energy $E$, angular momentum $L$, and number of particles $N$,  including terms for the conservation laws to be enforced. 
\begin{equation}
...
We can use the Fourier representation of the $\delta$ functions, particularly the the one conserving energy, to write the phase space volume as a contour integral in the complex plane.

\begin{equation}
Z(E,L,N) \propto  \int_C d\lambda e^{\lambda E} \int e^{ -\lambda(K+\Phi) }\delta^{(3)}(\mathbf{L} - \sum_i \mathbf{r}_i \times \mathbf{p}_i)\delta^{(3)}(\mathbf{r}_{cm})\left(\prod_{i=1}^N d^3r_id^3p_i\right)
\end{equation}
The contour of integration can
be deformed in the complex $\lambda$ plane using the method of steepest descents. The three conditions,
(a) that a saddle point exists, (b) that an observable not be of order exp(const.N),
...
of an athermal system, say of rigid links, this would just be that $E \propto 1/\beta$, where $\beta$ is the
value of $\lambda$ at the saddle point. Condition (b) is satisfied for the quantity of interest here,
the average radius of gyration. Condition (c) is also satisfied because we will see that the
average radius of gyration is smooth function of the temperature for $T$ for finite $T = 1/\beta$.
Thus we can drop the integration over $\lambda$ and replace $\lambda$ by the inverse temperature, $\beta$, and
consider the partition function $Z$ instead of the phase space volume integral (which
is simply related to the entropy.)

With the energy conserving delta function expanded in exponential form.
\begin{equation}
...
Z[\beta,L,N] = \frac{c}{L} \int_0^\infty \sin(kL)\zeta(\beta,k)kdk
\end{equation}
where $\zeta$ is a function of the magnitude of $k$ and inverse temperature  $\beta$ only. The constant $c$ is of no physical importance leading to:
\begin{equation}\label{rawzeta}
\zeta(\beta,k)= \int e^{ -\int^{Nl}_0 (\frac{Tmk^2}{2l} +\epsilon)(x(s)^2 + y(s)^2)+\epsilon z(s)^2 + \frac{3}{2l}  |\dot{\mathbf{r}}|^2  ds}\delta(\mathbf{r}_{cm}) D[\mathbf{r}(s)].%\delta \mathbf{r}(s) .
\end{equation}
...
\end{eqnarray}
The rescaling factors are based on the expected value of $L$ for the statistical model without conservation of angular momentum, and produce a set of dimensionless variables. This will allow us to compare chains of all sizes and temperatures on a common scale.
...
\zeta(\beta,k^\prime)= \int e^{-\int_0^{\sqrt{12}} (k^{\prime2}/2l + \epsilon^\prime)(x^\prime(s^\prime)^2 + y^\prime(s^\prime)^2 ) + \epsilon^\prime z^\prime(s^\prime)^2  + \frac{3}{2l} |\dot{\mathbf{r}^\prime}|^2 ds^\prime}\delta(\mathbf{r}^\prime_{cm}) D[ \mathbf{r^\prime}(s^\prime) ]
\end{equation}
...
\begin{equation}
\zeta(k^\prime)= \int e^{-\int_0^{\sqrt{12}} (k^{\prime2}/2l + \epsilon^\prime)(x^{\prime2} + y^{\prime2} ) + \epsilon^\prime z^{\prime2}  + \frac{3}{2l} |\dot{\mathbf{r}^\prime}|^2 + i x^\prime l_x + i y^\prime l_y + i z^\prime l_z ds^\prime}  dl_xdl_ydl_z D[ \mathbf{r^\prime}(s^\prime) ] .
\end{equation}
...
\begin{equation}
(\int e^{\sqrt{12}[ -(l_x^2 + l_y^2)/4(\frac{k^{\prime2}}{2l} +\epsilon^\prime) - l_z^2/4\epsilon^\prime]}dl_xdl_ydl_z)  (\int e^{-\int_0^{\sqrt{12}} (k^{\prime2}/2l + \epsilon^\prime)(x^{\prime2} + y^{\prime2} ) + \epsilon^\prime z^{\prime2}  + \frac{3}{2l} |\dot{\mathbf{r}^\prime}|^2 } D[ \mathbf{r^\prime}(s^\prime) ]
\end{equation}
...
After making this analogy, we can see that for any two fixed path endpoints, the functional integration will give the Green's function for a thermal harmonic oscillator. To obtain a sum over all paths, we simply need to integrate the Green's function $\rho$ ~\cite{feynman} over both its arguments,
\begin{equation}
\zeta_i = \int \rho_i(x_1,x_2)dx_1dx_2
\end{equation}
where $i = 1,2,3$ represent the $x$, $y$, and $z$, path integrals respectively.
The Green's function is easily integrated and the result is 
...
In this chapter, we considered the equilibrium properties of an ideal linear chain in a vacuum
...
approach of this chapter is not equivalent to this, and it is not clear
...
\caption{ 
A speculative model for fast streaming in \emph{Drosophila} oocytes, courtesy of Bill Saxton et al. 
...
This gives a diffusion timescale that is far too long to be effective for biological processes. 
...
motion is due to turbulence of the surrounding fluid. However at the low Reynold's numbers found in this system
, inertial effects are negligible and turbulence is
...
minus ends attached to the cell cortex(the inner layer of the cell wall) and their plus ends away from
...
%Mixing Paper
\chapter{Single Polymer Dynamics and Global Fluid Motion}
...
affects the above analysis, we need to consider the case of many microtubules working together.
Instead of a single microtubule isolated in fluid with an externally imposed flow field, consider all space filled with an infinite forest of microtubules all oriented
in the same direction. 

%%% Begin new explaination
Let us assume that the microtubules and impellers have managed to set up fast streaming and the fluid now has a long range global velocity $v$, brought about by the hydrodynamics of the motion of impellers running along the microtubules.
For the purposes of estimating the magnitude of this streaming velocity, assume that the microtubules have achieved a global correlation of direction, and have achieved a configuration in which they are largely straight and aligned with the fluid.
...
\end{equation}
Where $T(s)$ is the tension along the microtubule backbone, $F_k$ is the reaction force due to the walking kinesin, $v$ is the fluid velocity relative to the stationary microtubules, and $b_M$ is the local drag coefficient of the segment of microtubule, which can be approximated to within logarithmic corrections as that of a sphere with radius equal to the segment length, therefore in an fluid of viscosity $\eta$, $b_M = 6\pi \eta d$
...
b_I (v - v_0) =  -F_k
\end{equation}
Where $v_0$ is the kinesin walk speed, and $b_I$ is the local drag coefficient for the impellers, with $b_I = 6\pi\eta a$. 
$F_k$ is negative here as it is the oposite reaction force from that applied to the microtubule.
...
We will go on later to show that this small separation (on the order of the length of the kinesin), will give rise to a quadrupole like term in the fluid motion.
...
\end{equation}
And now take the limit as the total system size goes to infinity to give the generated fluid velocity due to a large patch of aligned microtubules.
\begin{equation}
...
This result tells us that if the microtubules manage to achieve a configuration such that they align into long straight bundles(as is seen in the oocyte during fast streaming), we would expect the fluid velocity generated by such a configuration to be a fraction of the maximum kinesin motor speed, with that fraction given by the relative drag coefficients of the microtubule segments and impellers.
Checking the limits of this ratio we find that if the drag from the microtubule were to be small, the fluid would move at the walk speed of the kinesin, and that if the drag from the microtubule were to be very large, the fluid velocity would be greatly suppressed but never quite zero.
...
While during fast streaming of the oocyte the microtubules are seen to be largely aligned with each other and the fluid motion, they do exhibit chaotic wavelike motion.
After estimating the fluid flow field one could expect from a field of microtubules carrying kinesin motors, we now need to investigate the behavior of the microtubule itself in the presence of such a field.
We begin with the equation for a single microtubule with an applied force tangent to the direction of chain. The position
...
Lastly we add a uniform flow field representing the velocity of the cytoplasm $v$ in the vicinity of the microtubule
...
line solution, so we can look for solutions of the form
\begin{equation}
\label{eq:travelling}
\bu(\sigma, \tau) = \bu_d(\sigma-v_d\tau) + \alpha \sigma {\hat k}.
\end{equation}
The term $\bu_d$ describes a transverse displacement waveform that travels along the backbone of the chain at velocity $v_d$, maintaining its shape.
Here $\alpha$ is the extent to which the chain is extended in the direction of the fluild motion.
Therefore we require
\begin{equation}
\label{eq:ud_constraint}
\frac{\partial \langle \bu_d \rangle}{\partial \sigma} = 0
\end{equation}
where the average is taken over the argument $\sigma-v_d\tau$.

Since $u_d$ is transverse, $\alpha$ sets the relation between $\sigma$ and the z-axis ($\hat{k}$), such that $v_z = \alpha v_d$.
If we choose $v_d = 1$ and in addition $\alpha = h$, we should obtain solutions which propagate along the fluid direction with the same unitless velocity as the fluid motion.
Looking back to our original units, $v_d = 1$ corresponds to a wave propagating along the backbone of the chain at velocity $f_k/\nu$ (roughly the kinesin walking speed).
Recall that $h$ is the rescaled unitless velocity of the external flow field, so $\alpha = h$ is the condition that the traveling wave form should propagate along the axis aligned with the globally imposed fluid velocity at the same speed as the fluid.
This will allow us to look for configurations that are stationary along that axis in the frame of the fluid.

Substituting Eq. \ref{eq:travelling} into Eq. \ref{eq:dimensionless} gives a left hand side
of $-v_d \partial \bu_d/\partial \sigma$ and a term of the form $-\partial \bu_d/\partial \sigma$ on
the right hand side, simplifying the equation to
...
There are an infinite number of solutions satisfying these conditions leading to different 
...
conformations of a microtubule, and motion in the $x-z$ plane(the plane of the hard wall, with the z axis being the direction of the global fluid field), corresponding to solutions similar in shape to cycloids.
...
height of the orbit be at $w_z = \alpha$. Therefore the radius perpendicular to the global fluid velocity $\hat k$ (the $w_x, w_y$ plane) is 
\begin{equation}
...
tangent vector rotates in the $x-y$ plane (the plane perpendicular to the fluid motion). 
...
\caption{ 
A plot of the value of $\alpha$, the wave propagation velocity, as a function of $1/G$, which is the energy of the analogous spherical pendulum.
}
\label{fig:AlphaVsE}
\end{center}
\end{figure}
...
\caption{ 
The rescaled radius of curvature for the helical configuration, $R_c/\rho_0$, versus the rescaled fluid velocity field $h=\alpha$.
...
Here the period is measured directly by timing repeated motion. The line is a guide for the eye.
(b)
...
The blue circles are the same data shown in Fig. \ref{fig:PeriodVSh}(a) by directly
measuring the period, which shows good agreement with the numbers produced by using Eq. \ref{eq:PvsRc}. Again the line is a guide for the eye.
}
\label{fig:PeriodVSh}
\end{center}
\end{figure}
...
Sec. \ref{subsec:CircularOrbits} with $h = \alpha = v = 0$. There the
...
%%%END MATH PAPAER
This analysis was extended to non-zero $v$ and as shown in section ~\ref{mathpaper}, does not
appreciably change the estimate we will give below for microtubule wave parameters in fast streaming oocytes. 


When the microtubule is tethered to an impenetrable surface (in this case, the oocyte cortex) and the external cytoplasmic streaming velocity $v$
...
lying close to the surface. For low enough $v$, travelling wave
solutions are close to prolate cycloids, meaning that the microtubule periodically loops back
on itself (see supplemental movie 2~\cite{SupplMovies}. This can be understood analytically. For sufficiently large $v$ it transitions to other states finally becoming 
two dimensional and looking close to a sinusoid (see supplemental movie 3~\cite{SupplMovies}. This is analyzed in detail in
section ~\ref{mathpaper}.
...
p(\br) &=& \int \frac{ \mathbf{f}(\br^\prime) \cdot (\br - \br^\prime)}{4 \pi | \br - \br^\prime|^3} d\br^\prime
\end{array}
\end{equation}
Where 
\begin{equation}
...
When used to describe the effect of applying force to a very small sphere of radius $a$, the Oseen tensor has higher order corrections in $a/\mathbf{r}$, where $\mathbf{r}$ is the distance from the sphere, but these begin at $(a/\mathbf{r})^3$ and should not play a significant role in cross polymer interactions.
...
The fluid velocity field can now be computed by applying the Oseen tensor and integrating over the force densities present in the system. 
The forces acting on the microtubule can be integrated by integrating over the backbone of the chain.
The forces acting on the impellers can be computed by integrating over a cylindrical sheath around the microtubule, giving us

\begin{equation}
\label{eq:fluidmodel}
 \mathbf{u}(\br) =  \int \left[ \mathbf{J}(\br-\br^\prime(s))[-C \frac{\partial^4 \br^\prime}{\partial s^4} + \frac{\partial}{\partial s}(T(s)\frac{\partial \br^\prime}{\partial s}) - \mathbf{f_k}] + \int  \mathbf{J}(\br-\br^\prime(s)+ \mathbf{a}(s)) \frac{ \mathbf{f_k}}{2\pi| \mathbf{a}|} \frac{\partial \br^\prime}{\partial s} d \hat{ \mathbf{a}} \right] \;ds
\end{equation}
...
Recall that $C$, $T(s)$, and $\mathbf{f_k}$ are the microtubule stiffness parameter, microtubule tension, and kinesin walk force respectively.
...
%thus begins the math to show its a quadrupole
\subsection{Kinesin Quadrupole}
...
In addition to this, we are assuming that the impellers are attached to the microtubule in a cylindrically symmetric fashion. This leads to a small scale force distribution that looks like a classic quadrupole configuration, as shown in figure \ref{fig:quadforce}.
...
In comparison, the tension can be decomposed into equal and opposite pairs between adjacent segments of the microtubule. However, we will find that they are not able to form a quadrupole like the kinesin forces do.
\begin{figure}[htp]
\begin{center}
\includegraphics[width=\hsize]{tenbal.png}
\caption{ 
While the forces connecting the microtubule backbone must balance, these forces are a) axial and lack the symmetric cancellation of the quadrupole and b) the separation between them is the basis for the characteristic length scale of the microtubule itself, and is thus fixed in relation to the size scale of the interaction.
}
\label{fig:tenforce}
\end{center}
\end{figure}
The separation between the two opposing forces is axial, and lacks the mirror symmetry needed to form the quadrupole. This means that it would give rise to a dipolar term.
...
As we have shown, the forces due to the kinesin motors directly do not contribute to the long range fluid motion, and the only remaining forces are those from microtubule backbone.
...
\end{equation}
This will give us the large scale fluid motion which will determine the large scale motion of the entire system. 
The coupling from the fluid back to the impeller/microtubule system will depend on the short range fluid interactions brought about by the kinesin motors; however, we still expect the long range motion of the microtubule/impeller system to match with the fluid. 
By symmetry, the difference in velocities will be axial, and the kinesin will still maintain a fixed relationship between the microtubule and impeller. From that we expect the fluid velocity to appear to be a weighted average of the microtubule and impeller velocity.
\begin{equation}
\label{eq:systemvelocity}
\mathbf{u}(\br) = \mathbf{v}_I(1-\delta) + \mathbf{v}_M \delta
\end{equation}
Where $\mathbf{v}_I$ and $\mathbf{v}_M$ are the impeller and microtubule velocities, respectively. If we use the fact that $\mathbf{v}_I - \mathbf{v}_M = v_k \frac{\partial \br}{\partial s}$, we find an expression for our microtubule velocity
\begin{equation}
...
This new boundary condition changes the Green's function used to construct the fluid velocity from the forces applied to the system. The determination of this Green's function is dependent on the specific boundary geometry, and potentially intractable.
...
Fortunately, the Green's function for the infinite wall case has already been solved\cite{DabrosVanDeVen}. The technique is  similar to that of the method of image charges in classical electromagnetism, with the added complication of the sources being vector quantities instead of scalers. This vector nature of the sources gives rise to a number of corrections to the simple reflection.
...
\begin{equation}
\label{eq:oseenwall}
\mathbf{J}^w(\mathbf{x}, \mathbf{y})\cdot \mathbf{f} = \frac{1}{8\pi\mu \br^*} [ 
...
We ran this simulation using the same parameters for a single microtubule with and without the fluid interaction, again using a random starting configuration and tethering the minus end of the microtubule to a fixed point in space. 
...
\caption{ 
Simulation results for a single polymer without fluid interaction(blue) and with fluid interaction(orange), with all other parameters held the same. Both polymers are anchored at the center of the image, near the center of the circular configuration. The images are super imposed, but the polymers were simulated separately. They are qualitatively the same, though the radius is slightly larger with fluid interactions activated.
}
\label{fig:singtest}
\end{center}
\end{figure}
...
The spacing between the individual anchors can be varied, but will never be close enough for their hardcore radii to overlap.
...
The microtubules are placed with their anchor points in a plane parallel to the hard wall, and initialized with a random configuartion subject to the constraint that they not pass through the wall. 
...
In the cell, the hard wall is actually believed to be a complex mesh work of actin filaments and other proteins, to which the microtubules are attached.
This mesh is potentially porous to the fluid and there is additional evidence that the microtubule anchors extend out into the cytoplasm during fast streaming.
For this reason we study microtubule patches that are at some small fixed distance away from the hard wall boundary.
The separation between the plane of the hard wall and the plane of anchors, $h$ is a second new length scale.
...
These correlations arise as the motion of fluid near a microtubule cause that microtubule to align with the preexisting flow.
An image showing the grouping of microtubules during fast streaming can be seen in Fig. \ref{fig:biostream}.
\begin{figure}[htp]
\begin{center}
\includegraphics[width=\hsize]{fast.png}
\caption{ 
An image taken with a two photon microscope of a \emph{Drosophila} ooctye during fast streaming. The Oocyte was treated so that the microtubules would fluoresce and traces of them can be seen. In this view, the cytoplasm flows from the top left corner of the image toward the bottom right corner. In the regions of fast fluid motion, the microtubules can be seen to align with the direction of the fluid motion. Image care of Bill Saxton et. al.
}
\label{fig:biostream}
\end{center}
\end{figure}
...
However, since the microtubules align with a local flow field and then, once aligned, they contribute to that flow, it is plausible that a type of field amplification occurs.
...
\caption{ 
A typical starting configuration for a microtubule patch, viewed from directly above the plane of the hard wall. The microtubules are anchored at their minus ends in a square grid pattern above the wall by the same distance as the separation between the anchor points. The grid of anchor points occupies the majority of the view, though it is slightly shifted to give a better view of the microtubules.
}
...
\caption{ 
A typical configuration after correlation, viewed from an angle to better show the planar nature of the solution. The grid of anchor points is significantly farther from the hard wall boundary in this simulation than was used in the previous image, in order to facilitate faster correlation. The microtubules are anchored at their rightmost endpoint in this image, and the fluid flow is left and away from the viewer. The microtubules on the edges of the patch can be seen to execute a helical pattern reminiscent of the single microtubule dynamics.
}
...
\begin{figure}[htp]
\begin{center}
\includegraphics[width=\hsize]{noodles.png}
\caption{ 
A system that never achieves directed fast streaming. It does show temporary local correlations, but these are not aligned in any one direction.
}
\label{fig:noodles}
\end{center}
\end{figure}
...
Whether a system will reach the correlated directed fast streaming phase is determined by a number of parameters that effect the relative strength of the forces in the system.
These parameters can be the internal properties of the microtubule/impeller system, or properties of the arrangement of the microtubules in the patch.
...
As can be seen from Eq. \ref{eq:discretesum} and  \ref{eq:oseentensordef}, there is a global factor of $\mu^{-1}$ on all of the driving force terms.
Changing the viscosity on any given system will then inversely effect the fluid velocity field from the any given set of forces.
...
The motion of the microtubule itself is given by Eq. \ref{eq:discretevel}, which includes a term only proportional to the kinesin walk speed.
...
\subsection{Hysteresis of Streaming}
...
For instance, it is believable that a system could be just barely able to support streaming with the microtubules aligned, but that such a system would be unable to spontaneously generate that alignment.
...
In the cell, microtubules are observed to be uncorrelated in the slow streaming phase, while during the fast streaming phase they are observed in correlated bundles.
...
\section{Biologicaly Accessible Parameters for Streaming}
...
These studies show that in the stages of slow streaming, the microtubules mostly anchor tightly against the cell cortex by thick bundles of actin, another polymer used in the cytoskeleton.
These bundles are thought to hold the microtubule ends in a thin layer very close to the cell wall, something which could prevent the production of correlated fast streaming.
...
\section{Conclusion}
We have investigated a potential model for both the slow and fast streaming phases in the cytosplamic streaming found in \emph{Drosophila} oocytes.
We found that even for patches including a relatively small number of microtubules, there exists a range of parameters for which the system will achieve a correlated streaming phase capable of providing the large scale flow field.
These parameters include microtubule density, fluid viscosity, the characteristic buckling length of the microtubules, and the distance of the anchor points from the hard wall boundary.
These parameters could be used as biological controls for the transition from slow to fast streaming at a precise time during the development of the oocyte.

Further investigations into the model would include looking for a functional form for the dependence of the phase transition on the key parameters.
What condition must be met in order for the system to always tend to fast streaming?
Is simply moving the anchors away from the cell wall enough? Or is a more complex mechanism required?

\chapter{Conclusion}
In this work we have explored two different ways in which the theory of polymers can be applied to help understand complex biological systems.
First, we looked at the statistics of polymers isolated in a vacuum. 
An analytic expression for the dependence of the radius of gyration on the angular momentum of the polymer was obtained, and then compared to the results of a dynamic simulation.
The simulation was then used to investigate the anisotropy of the spatial distribution of the polymer caused by its rotation.
These results could potentially be used to help identify large biological polymers such as proteins or DNA.
They could also be useful in further study of the internal distributions of mass within the polymers.

Second, we looked at a simple model for describing the motion of microtubules and cytoplasm in \emph{Drosophila} oocytes.
Considering a single microtubule suspended in a high viscosity flow allowed for an analytic treatment of potential steady state solutions.
These solutions are generally helical except in the presence of a hard wall, in which case they are still generally curved with some characteristic period.
The equations of motion allowed for solutions of arbitrary scale, with the actual scale chosen by the boundary conditions; in this case a fixed location at one end and free at the other.
The system was simulated numerically, which results that conformed to the analytic forms, and allowed for an estimation of the scale parameter chosen.

The single polymer analysis assumed an externally imposed flow field, so we then investigated if a group of microtubules could create such a field.
Patches of microtubules were simulated using a modified Oseen tensor to model the hydrodynamic interactions between the microtubules in the presence of the nearby cell wall.
These simulations displayed spontaneous correlation into a directed streaming phase for a wide range of parameters.
Several of these parameters could be tuned to prevent the system from reaching the fast streaming phase, with the simplest being the distance of the anchor points from the hard wall.
This change of position relative to the wall is observed between the slow and fast streaming phase in the oocyte itself, and suggests a possible mechanism for controlling the onset of fast streaming.
The simple model used here shows many of the features seen in the oocyte, and could provide a useful tool for discovering more about how the cell functions.
...
\bibitem{DabrosVanDeVen}
T. Dabros and T. G. M. van de Ven, ``Hydrodynamic Interactions Between Two Spheres Near a Solid Plane" Int. J. Multiphase Flow Vol. 18, No. 5, pp. 751-764, (1992)
