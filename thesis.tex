...
%Mixing Paper
\section{Transition to Biopolymers}
...
Cytoplasmic streaming is a process by which the contents of a single cell are rapidly moved on a scale that covers the entire cell. While most often seen in plants, whose cell walls restrict outside purturbations of the interior fluid, the effect is seen in a wide variety of cells. 
...
The specific instance of this streaming we studied is the fast streaming present in the developement of Drosophila oocytes. This streaming happens at a specific time during the developement cycle, just before cytoplasm from nearby nurse cells is dumped asymetrically into the oocyte.

[*** diagram of oocyte nurse cell arrangement***]

A great deal of work has been done to determine what components are involved in this particular instance of streaming

The specific timing of this streaming emphasizes the importance of not just the streaming, but also in getting the streaming to turn on and off quickly. Many experiments have been done to explore the effect of miss-timed or nonexistant streaming


It is possible that this triggering is done via some biochemical means, but its is aslo possible that the streaming activation is done through a more physical means.
...
b_M v = \frac{\partial}{\partial s}T + F_k
\end{equation}
...
b_I (v - v_0) + b_M v = -\frac{\partial}{\partial s}T
\end{equation}
...
\begin{equation}
\label{eq:mfflow}
v = \int -C \frac{(b_I (v - v_0) + b_M v)}{|\br|} d^3\br = -2\pi C R^2 (b_I (v - v_0) + b_M v)
\end{equation}
Where $C$ is proportionality constant determined by the fluid interaction strength which will not effect the final result of this analysis.
Solve this for $v$
\begin{equation}
\label{eq:mfflowsol}
v = \frac{2\pi C R^2 b_I}{2\pi C R^2(b_I + b_M) + 1} v_0
\end{equation}
...
%%%end old explaination


%%% Begin old explaination
...
\begin{equation}
\label{eq:fluidmodel}
\bf u(\br) =  \int \bf J(\br-\br^\prime)[-C \frac{\partial^4 \br^\prime}{\partial s^4} + \frac{\partial}{\partial s}(T(s)\frac{\partial \br^\prime}{\partial s}) - f_k \frac{\partial \br^\prime}{\partial s}] + \bf \int J(\br-\br^\prime+\bf a) \frac{f_k}{2\pi|\bf a|} \frac{\partial \br^\prime}{\partial s} d\bf \hat{a} ds
\end{equation}
where $\bf u$ is the fluid velocity and $a$ is a vector pointing radially outward from the microtubule axis to the impeller location. 
...
The microtubules have a significant stiffness which leads to a characteristic buckling length, which sets a size scale for features of the microtubule motion. Compared to this size scale, the impellers are very small, and bound very tightly to the microtubule backbone. 
...
\delta\bf u(\br) = \frac{1}{8\pi\mu}\int \frac{\bf f}{|\br + \bf a|} + \frac{(\br+\bf a) ((\br + \bf a) \cdot \bf f)}{|\br + \bf a|^3} d\hat{a} - \frac{\bf F}{|\br|} - \frac{\br (\br \cdot \bf F)}{|\br|^3}
\end{equation}
Where $\delta \bf u(\br)$ is the contribution to the fluid velocity at position $\br$ from a differential segment of the microtubule impeller system, $a$ is the vector pointing from the center of the microtubule to a point in the impeller sheath, and $f$ and $F$ are the force densities produced by the kinesin interaction. Note that $F$ is a linear force density and $f$ is a force per unit area, due to the radial nature of the coupling.
...
Due to the small size of the kinesin molecule, for long range interaction on the order of the length of the microtubule, or even characteristic curvatures of the microtubule, we should have $a << r$, where $a$ and $r$ are the magnitudes of $\bf a$ and $\br$ respectively. We can pull out a factor of $r$ to obtain these expressions in terms of a unit-less small parameter $\epsilon = a/r$.
\begin{equation}
...
\delta\bf u_0(\br) = \int \frac{\bf f}{r|\hat{r}|} + \frac{(\hat{r}) ((\hat{r}) \cdot \bf f)}{r|(\hat{r})|^3} d\bf \hat{a} - \frac{\bf F}{|\br|} - \frac{\br (\br \cdot \bf F)}{|\br|^3}
\end{equation}
...
\delta \bf u(\br) = \frac{1}{r} \int  \bf f[(\hat{r} + \epsilon\hat{a})^2]^{-1/2} d\bf \hat{a} +(\hat{r} + \epsilon\hat{a})(\hat{r} + \epsilon\hat{a})\cdot \bf f [(\hat{r} + \epsilon\hat{a})^2]^{-3/2} d\bf a - \frac{\bf F}{|\br|} - \frac{\br (\br \cdot \bf F)}{|\br|^3}
\end{equation}

\begin{equation}
\begin{array}{rcl}
\label{eq:forcedensityfirstderiv}
\frac{d}{d\epsilon} \delta \bf u(\br) &=& \frac{1}{r} \int 
-\bf f [(\hat{r} + \epsilon\hat{a})^2]^{-3/2} (\hat{r} + \epsilon\hat{a})\cdot \hat{a} \\
&+&\hat{a}(\hat{r} + \epsilon\hat{a})\cdot \bf f [(\hat{r} + \epsilon\hat{a})^2]^{-3/2} \\
&+& (\hat{r} + \epsilon\hat{a})(\hat{a}\cdot\bf f)[(\hat{r} + \epsilon\hat{a})^2]^{-3/2}\\
&-& 3 ((\hat{r} + \epsilon\hat{a})(\hat{r} + \epsilon\hat{a})\cdot\bf f [(\hat{r} + \epsilon\hat{a})^2]^{-5/2} (\hat{r} + \epsilon\hat{a})\cdot \hat{a}
\end{array}
\end{equation}
\begin{equation}
\begin{array}{rcl}
\label{eq:forcedensitylimit}
\lim_{\epsilon\to 0} \frac{d}{d\epsilon} \delta \bf u(\br) &=& \frac{1}{r} \int 
-\bf f [(\hat{r})^2]^{-3/2} (\hat{r})\cdot \hat{a} \\
&+&\hat{a}(\hat{r})\cdot \bf f [(\hat{r})^2]^{-3/2} \\
&+& (\hat{r})(\hat{a}\cdot\bf f)[(\hat{r})^2]^{-3/2} \\
&-& 3 ((\hat{r})(\hat{r})\cdot\bf f [(\hat{r})^2]^{-5/2} (\hat{r})\cdot \hat{a}
\end{array}
\end{equation}
...
How does this compare with the other forces in the system? Well, the tension can be decomposed into equal and oposite pairs between adjancent segments of the microtubule.
\begin{figure}[htp]
\begin{center}
...
\end{center}
\end{figure}
However, for this force the cancelations found in the kinesin quadrapole run into some problems. For one, the seperation between the two oposing forces is axial, and lacks the mirror symetry needed to form the quadrapole. This means that it would at best form a chain of dipoles. 
The real problem here, for both the tension and the stiffness, is that the corresponding small parameter needed to form the multipole cancelation is directly tied to the characteristic length scale of the microtubule curvature; indeed, the curvature is defined by these forces. 
...
\bf u(\br) = \bf v_I(1-\delta) + \bf v_M \delta
\end{equation}
Where $\bf v_I$ and $\bf_vM$ are the impeller and microtubule velocities, respectively. If we use the relation between these two velocities we find an expression for our microtubule velocity
\begin{equation}
\label{eq:mtubevelocity}
\frac{d \br}{dt} = \bf v_M = \bf u(\br) - (1-\delta)v_k \frac{\partial \br}{\partial s}
\end{equation}
...
\frac{d \br}{dt} = \bf u(\bf r ) - \bf v_k \frac{\partial \br}{\partial s}
\end{equation}
...
\bf T(\br_i) = k_T ( |\bf d_{i-}| - l )\bf \hat{d_{i-}} + ( |\bf d_{i+}| - l )\bf \hat{d_{i+}}
\end{equation}
With 
\begin{eqnarray*}
\bf d_{i-} &= \br_{i-1} - \br_i \\
\bf d_{i+} &= \br_{i+1} - \br_i
\end{eqnarray*}
...
\bf C(\br_i) =  - k_C[ (r_{i+2} - r_i) - (r_i - r_{i-2})]
\end{equation}
...
This was implemented as a inverse fourth power law:
\begin{equation}
\label{eq:discretehardcore}
\bf H(\br_i) = \sum(1 - \frac{d^4}{|\br_i - \br_j|^4})*(\br_i - \br_j)
\end{equation}
...
\frac{d \br_i}{dt} = \bf u(\br_i) - v_k (\br_{i-1} - \br_{i+1})
\end{equation}
...
Fortunately, the Green's function for the infinite wall case has aloready been solved. The technique is  similar to that of the method of image charges in classical electromagnatism, with the added complication of the sources being vector quantities instead of scalers. This vector nature of the sourcees gives rise to a number of corrections to the simple reflection.
Unfortunately, it is somewhat complex, though luckily not of higher order scaling than the unmodified Oseen tensor. The interaction tensor is now:

[***massive equation ***] 

For a wall located at $z=0$
