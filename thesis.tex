...
\delta\bf u(\br) = \int \frac{\bf f}{|\br + \bf a|} + \frac{(\br+\bf a) ((\br + \bf a) \cdot \bf f)}{|\br + \bf a|^3} d\bf a - \frac{\bf F}{|\br|} - \frac{\br (\br \cdot \bf F)}{|\br|^3}
\end{equation}
...
Due to the small size of the kinesin molecule, for long range interaction on the order of the length of the microtubule, or even characteristic curvatures of the microtubule, we should have $a << r$, where $a$ and $r$ are the magnatudes of $\bf a$ and $\br$ respectively. We can pull out a factor of $r$ to obtain these expressions in terms of a unitless small parameter $\ep = a/r$.
...
\subsection{The discrete modele}
As with the single microtubule case, the microtubules are modeled as a chain of discrete points. Unfortunatly, the use of the long range fluid interactions prevents us from using pure rigid links, so this model uses a backbone comprised of stiff spring links. The spring constant is chosen to be as strong as possible for the given timestep.
