...
Polymer dynamics is an exciting field with applications across many different disciplines. 
While the bulk statistical properties have been studied for quite some time, improvements in microscopic techniques have allowed investigations into individual and dynamical properties.
...
Biology makes extensive use of polymers and their properties in a huge number of processes vital to living.
...
Drosophila are extensivly studied and provide a well understood basis on which to perform variations to explore the parameters involved in streaming.
...
\caption{ 
A speculative model for fast streaming in Drosophila oocytes, care of Bill Saxton et al. 
...
The spaceing between the individual anchors can be varried, but will never be close enough for their hardcore radii to overlap.
This grouping of microtubules exhibits additional behaviors not found in the single microtubule system.

\subsection{Important Lengthscales}
...
Within this plane, the anchors have a charateristic seperation,$R_s$, and this seperation defines one of the new length scales in the system.
The plane of anchors is parallel to the hard wall, but not directly on it. 
The seperation between the hard wall and the plane of anchors, $h$ is a second new length scale.
These two new lengths are usually set to be less than the characteristic buckling lentgh of the microtubule, $R_b$, in order to more closely match what is seen in the biological data.
We will see that these two length scales play an important role in determining the corellation behavior of the system.
...
\caption{ 
An image taken with a two photon microscope of a Drosophila ooctye during fast streaming. The Oocyte was treated so that the microtubules would flouress and traces of them can be seen. In the regions of fast fluid motion, the microtubules can be seen to align with the direction of the fluid motion. Image care of Bill Saxton et. al.
}
\label{fig:biostream}
\end{center}
\end{figure}
...
When the simulation is run with a small number of microtubules placed with their anchors near each other (less than the buckling length), short range correllation almost always emerges.
Using a random walk starting condition, the microtubules initially fall into seemingly independant circular solutions simialr to those in the single microtubule case.
However, after a few periods of revolution the microtubules eventually collapse into one circular solution, as seen in Fig. \ref{fourpoly}
...
\caption{
(a)
A typical starting configuration of four microtubules, with $R_s = 2$ and $h = 2$.
(b)
...
This artifact of the somewhat artificial initial conditions shouldn't be relavent to the actual biological system, so these steps are generally not shown.
...
There are a fairly wide range of parameters for which, regardless of the random inititialization, the microtubule patch will self organize into a stable directed flow.
...
\caption{ 
A typical configuration after correlation, viewed from an angle to better show the planar nature of the solution. The microtubules on the edges of the patch can be seen to execute a helical pattern reminecent of the single microtubule dynamics.
}
...
It is important to re-emphasize that there is no externaly imposed fluid motion in this simulation, and that the linear configuration is brought about by the fluid interactions between the microtubules.
The direction chosen for streaming is dependent on the exact nature of the random initial conditions, whuch are not guarenteed to be perfectly isotropic.
...
When the fluid interaction forces are not enough too overcome the kinesin reation force, the microtubules will buckle and begin rotating around their anchor points.
...
Regions of correlated microtubules can develope, as see in Fig. \ref{fig:noodles}
...
Fig. \ref{fig:walldist} shows a sequence of seperate patches, each at different distances from the wall. 
...
(d)Patch at $h=1$, shows no directed streaming, only correlation in cicular motions
}
\label{fig:walldist}
\end{center}
\end{figure}
...
Eventually, the wall decreases the interaction stregth significantly enough that the nucleation of the linear streaming regions cannot occur.
...
The cell may undergo some change of internal parameters to cross the phase boundry from uncorrelated slow streaming to correlated fast streaming.
...
However, if the cell is indeed crossing this phase boundry in the transition from slow to fast streaming, it can only do so using parameters that can be changed in the cell in a relatively short timescale.
Some of the parameters can be artificially changed to try to isolate their effects.

Studies have been done to investigate the nature of the anchoring of the microtubules to the cell cortex.
...
This corresponds very closely to the simulation result that fast streaming can be supporessed by anchoring the microtubules tightly to the hard wall.
