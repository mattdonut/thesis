...
Advances in molocular manipulation allows for more detailed more detailed analysis of large biological molecules, such as protein and DNA.
...
While current systems use time-of-flight metheds to examine the mass of these polymers, photon scattering techniques could be used to study the their average size and aspect ratio.
Here we report the development of an analytic expression for the dependance of the radius of gyration of a polymer suspended in vacuum to its angular momentum, it the agreement of that expression with numerical simulations.

Polymer dynamics can also be studied in vivo, with a great deal of the behavior captured by some simple models.
Here we study cytoplasmic streaming in \emph{Drosophila} oocytes. The oocyte displays two stages of fluid movement; a slow stage that is generally chaotic and localized and a fast stage that is highly correlated across the entire cell.
...
We also investigate the behavior of many microtubule using simulations which include hydrodynamic interactions.
These simulations have parameter ranges where they exhibit chaotic localized motion and ranges where they exhibit globaly correlated directed motion.
The parameters that cause a transition from one phase to the other correspond to changes observed in the oocyte in the transition from slow to fast streaming.
...
Thanks to Jennifer Raab for all of her help throughout my graduate carrer, especially for helping me find funding.
...
We found that even for patches including a relatively small number of microtubules, there exists a range of parameters for which the system will acheive a corrlated streaming phase capable of providing the large scale flow field.
These parameters include microtubule density, fluid viscosity, the chatacteristic buckling length of the microtubules, and the distance of the anchor points from the hard wall boundry.
...
Further investigations into the model would include looking for a functional form for the dependance of the phase transition on the key parameters.
...
An analytic expression for the dependance of the radius of gyration on the angular momentum of the polymer was obtained, and then compared to the results of a dynamic simulation.
...
These solutions are generally helical except in the presence of a hard wall, in which they are still generally curved with some characteristic period.
The equations of motion allowed for solutions of arbitrary scale, with the actual scale chosen by the boundry conditions; in this case a fixed location at one end and free at the other.
The system was simulated numerically, whiched conformed to these analytic forms, and allowed for an estimation of the scale parameter chosen.

The single polymer analysis assumed an externally imposed flow field, and we then studied if a group of microtubules could create such a field.
To study this, patches of microtubules were simulated using a modified Oseen tensor to model the hydrodynamic interactions between the microtubules in the presence of the nearby cell wall.
These simulations displayed spontaneos corellation into a directed streaming phase for a wide range of parameters.
...
There is biological evidence that these anchor points move away from the cell cortex during the transition from slow to fast streaming, which is in agreement with what is seen in the simulation.
