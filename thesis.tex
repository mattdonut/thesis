...
For the purposes of estimating the magnatude of this streaming velocity, assume that the microtubules have achieved a global correlation of direction, and have achieved a configuration in which they are largely straight and aligned with the fluid.
...
the right hand side. If we choose $v_d = 1$ and in addition $\alpha = h$, the equation simplifies to
...
the supplemental materials~\ref{mathpaper}.
...
These correlations arise as the motion of fluid near a microtubule cause that microtubule to align with the prexisting flow.
...
However, since the microtubules align with a local flow field and then, once aligned, they contribute to that flow, it is plausable that a type of field amplification occurs.
...
\caption{ 
A typical starting configuration for a microtubule patch, viewed from directly above the plane of the hard wall. The microtubules are anchored at their minus ends in a square grid pattern above the wall by the same distance as the seperation between the anchor points. The grid of anchor points occupies the majority of the view, though it is slightly shifted to give a better view of the microtubules.
}
...
\caption{ 
A typical configuration after correlation, viewed from an angle to better show the planar nature of the solution. The grid of anchor points is significantly farther from the hard wall boundry in this simulation than was used in the previous image, in order to facilitate faster correlation. The microtubules are anchored at their rightmost endpoint in this image, and the fluid flow is left and away from the viewer. The microtubules on the edges of the patch can be seen to execute a helical pattern reminiscent of the single microtubule dynamics.
}
...
Whether a system will reach the correlated directed fast streaming phase is determined by a number of paremeters that effect the relative strength of the forces in the system.
...
These studies show that in the stages of slow streaming, the microtubules mostly anchor tightly against the sell cortex by thick bundles of actin, another polymer used in the cytoskeleton.
