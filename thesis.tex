...
\end{center}
\end{figure}
The MALDI technique involves embedding the large polymers of interest into a matrix of much smaller molecules, commonly acids to promote ionization durning the vaporization process. 
Once the analate and matrix are co-crystalized, they are placed into a vacuum and bombarded with a laser, usually in the UV. 
...
\caption{ 
The impellers are carried along the microtubule by the kinesin motors. Kinesin is a small protein, so the impellers are aranged very close to the microtubule, which is itself very thin.
}
\label{fig:quadforce}
\end{center}
\end{figure}
...
(c)
The kinesin motors act on both the microtubule and the impellers, and both of these force densities are transmitted to the fluid. These force densities arise due to a local coupling. and therefore are related by Newton's third law.
}
\label{fig:RvsL}
\end{center}
\end{figure}
...
\delta\bf u(\br) = \int \frac{\bf f}{|\br + \bf a|} + \frac{(\br+\bf a) ((\br + \bf a) \cdot \bf f)}{|\br + \bf a|^3} d\hat{a} - \frac{\bf F}{|\br|} - \frac{\br (\br \cdot \bf F)}{|\br|^3}
\end{equation}
...
As we can see, each term in the integrand is an odd function of $\hat{a}$, and the integral is around the whole circle, so this vanishes. This means that the leading order term from the kinesin coupling force density is going to be surpressed by two factors of the small parameter $\epsilon$ relative to the other force densities in the system. As such, for long range considerations we can safely ignore the forces caused directly by the kinesin molecule itself do to it's small size. That is not to say that the kinesin does not effect the system at all.
